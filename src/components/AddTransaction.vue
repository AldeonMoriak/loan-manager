<template>
  ...
  <ModalContainer :is-shown="props.isShown">
    <form
      @submit.prevent="insertTransaction"
      ref="target"
      ...
      <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div class="flex flex-col">
          <div class="text-xl font-bold text-gray-500 mb-4">
            {{ store.dir === "rtl" ? "افزودن قسط" : "New Transaction" }}
          </div>
          <div id="loans mt-5">
...
async function insertTransaction() {
  ...
  if (form.name.length <= 3) {
    alertMessage.value =
      store.dir === "rtl"
        ? "عنوان خیلی کوچیکه!"
        : "Please make your transaction a little more descriptive";
  ...
  if (!form.amount || form.amount === 0) {
    alertMessage.value =
      store.dir === "rtl"
        ? "مقدار پرداختی وارد نشده"
        : "Please enter a valid value for transaction amount";
    alertType.value = "error";
    isAlertShown.value = true;
    return;
  }
