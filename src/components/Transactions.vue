        ...
        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
          <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-gray-50 dark:bg-blue-gray-300">
              <tr>
                <th v-for="(header, index) in headers" scope="col" :key="index" :class="
                  store.dir === 'rtl'
                    ? 'text-right text-sm'
                    : 'text-left text-xs'
    ...
    header
            ...
            <tbody class="bg-white divide-y divide-gray-200 font-poppins">
              <tr v-for="(row, index) in props.rows" :key="index"
                :class="index % 2 !== 0 ? 'bg-white dark:bg-blue-gray-200' : 'bg-gray-100 dark:bg-blue-gray-100'">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900" :class="store.dir === 'rtl' ? 'text-right' : 'text-left'"
                    dir="ltr">{{ store.dir === 'rtl' ?
                ...
                <td :class="store.dir === 'rtl' ? 'text-left' : 'text-right'"
                  ...
                  <a @click="onClickDelete(row, index)"
                    class="cursor-pointer text-red-600 py-1 px-2 hover:(text-red-900 bg-red-100 rounded)">{{ store.dir
                        === "rtl" ? "حذف" : "Delete"
                    }}</a>
                  <a id="edit-button" @click="onClickEdit(row, index)"
